...
- **Search look-up slices (#811)** — the Search journey runs set-pinned name queries (name + set-name), a different cache key than the browse `set.id:"mcd19"` path, so two extra structural + pricing slices warm the `Pikachu | McDonald's Collection 2019` / `Eevee | McDonald's Collection 2019` look-up lines `search-export.spec.ts` pastes. The line text drives the cache key, so the spec's lines must match what generated the slices.
