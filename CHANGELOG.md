...
- DevOps: **Conventional Commits enforcement + release-please drafts the next version-bump PR** ([#68](https://github.com/mgzwarrior/mgz-pkmn/issues/68)). New CI workflow `Conventional Commits` (`.github/workflows/conventional-commits.yml`) walks every non-merge commit on a PR and validates the subject line against `<type>(<scope>)?!?: <subject>` in pure shell — same shape as the existing `dco.yml`, no Node / Python deps to bootstrap. Allowed types are the Angular set plus `revert`. The same vocabulary is enforced locally at commit-msg time via gitlint (`.gitlint`, wired through `.pre-commit-config.yaml`) so a green local commit is a green CI check. New `.github/workflows/release-please.yml` (`googleapis/release-please-action@v4`) watches `main` and opens a version-bump PR whenever a release-worthy commit lands, rotating `[Unreleased]` into a new versioned CHANGELOG section and bumping `pyproject.toml`. PR-only mode (`skip-github-release: true`) so the existing `release-on-version-bump.yml` → `release.yml` chain still owns tagging + PyPI publishing + PEP 740 attestation. The bot's PR is the canonical release PR — the releasing agent / human extends it to cover the other version surfaces (`api/pyproject.toml`, `web/package.json`, `site/package.json`, `CITATION.cff`) and applies the CHANGELOG consolidation pass ([#571](https://github.com/mgzwarrior/mgz-pkmn/issues/571)) before merging. `docs/contributing.md` grows a new "Commit messages" section with the format, allowed types, scope conventions, examples, and local-enforcement instructions; `CLAUDE.md` adds the rule to the agent workflow. The retroactive one-time CHANGELOG cleanup pass is tracked separately as [#583](https://github.com/mgzwarrior/mgz-pkmn/issues/583).
- API: **Collections data model rework — foundational schema slice of the collections rethink** ([#574](https://github.com/mgzwarrior/mgz-pkmn/issues/574), epic [#501](https://github.com/mgzwarrior/mgz-pkmn/issues/501), [ADR-0025](docs/adr/0025-collections-data-model-rework.md)). New Alembic revision `c01ec71011a7` adds `collection_items.quantity` (vendor multiples), promoted card-identity columns on both `collection_items` and `wishlist_items` (`card_set_id`, `card_number`, `card_name`, `card_rarity`, `card_types_json`, `card_image_url`, `price_snapshot`, `priced_at`) backfilled from existing `card_json`, `collection_items.added_via` provenance, `collections.kind` + `source_set_id` + `rule_json` (manual stays the default; set-based and dynamic kinds ride [#506](https://github.com/mgzwarrior/mgz-pkmn/issues/506)), `wishlist_items.acquired_at` + FK to the resulting collection item (powers the wishlist → collection promote in [#504](https://github.com/mgzwarrior/mgz-pkmn/issues/504)), `wishlists.target_date`, and a new first-class `collection_snapshots` table that backs the value-over-time chart on the insights dashboard ([#575](https://github.com/mgzwarrior/mgz-pkmn/issues/575)) and the set-ID-card progress view ([#508](https://github.com/mgzwarrior/mgz-pkmn/issues/508)). Indices on `(card_set_id, card_number)` for both item tables so the cross-collection ownership badge ([#576](https://github.com/mgzwarrior/mgz-pkmn/issues/576)) and library-aware swipe exclusion ([#581](https://github.com/mgzwarrior/mgz-pkmn/issues/581)) are real index scans. The single payload extractor lives in `api/db/card_payload.py` and is shared between the insert routes and the migration backfill so a future pokemontcg.io shape drift is a one-file change. `POST /api/v1/collections/{id}/items` now accepts optional `quantity` and `added_via`; response envelopes on both collection and wishlist items grow the promoted fields. `docs/collections.md` and `docs/wishlists.md` document the new request / response shapes. ADR-0025 captures the design rationale — separate-but-linked wishlists, promoted card identity as denormalization, lifecycle as data not flag, first-class snapshots — and amends [ADR-0013](docs/adr/0013-sqlite-persistence-for-runs-collections-wishlists.md) without superseding it. Pinned by `tests/test_card_payload.py` against a representative pokemontcg.io payload plus migration-roundtrip + backfill-correctness coverage in `tests/test_collections.py`.

### Changed

- Web: **Unified Library destination consolidates the four "saved stuff" surfaces** ([#528](https://github.com/mgzwarrior/mgz-pkmn/issues/528)). The desktop saved-searches sidebar, the recent-searches panel under the editor, and the header chips that opened the Collections / Wishlists modals all rolled into one `LibraryPanel` with four internal tabs (Searches / Recent / Collections / Wishlists). On desktop the panel takes the persistent left-rail slot the saved-searches sidebar used to occupy and keeps the same collapse-to-icon-strip affordance. On mobile it sits as a collapsed-by-default accordion above the editor so the editor stays the hero on short viewports; expanding reveals the same four-tab surface. The header chips for Collections and Wishlists are gone — both surfaces now live inside the Library — and `RecentRuns` no longer renders under the editor. Each tab keeps the existing per-object behavior (click a saved search to hydrate input + rows + `view_state`, click a recent entry to re-run, see collection / wishlist counts) so this is an IA / surface rewire rather than a behavior change. `SavedSearchesSidebar`, `RecentRuns`, `CollectionsModal`, and `WishlistsModal` are removed; new per-tab components (`LibrarySearchesTab`, `LibraryRecentTab`, `LibraryCollectionsTab`, `LibraryWishlistsTab`) carry the extracted content and ship with equivalent test coverage. Unblocks the mobile bottom-tab nav ([#519](https://github.com/mgzwarrior/mgz-pkmn/issues/519)) and the desktop side-by-side workspace ([#522](https://github.com/mgzwarrior/mgz-pkmn/issues/522)) which both depend on a single Library destination.

### Added

- Design: **Styleguide published to GitHub Pages at `styleguide.mgz-pkmn.com`** ([#547](https://github.com/mgzwarrior/mgz-pkmn/issues/547), [#567](https://github.com/mgzwarrior/mgz-pkmn/issues/567)). New `.github/workflows/pages.yml` stages `design/styleguide/`, `design/tokens/`, and `assets/` into a deploy artifact and publishes the tree on every push to `main` that touches `design/` or `assets/`. The root index redirects to `design/styleguide/index.html`; the relative `../tokens/colors_and_type.css` and `../../assets/*.svg` references the cards already use resolve unchanged. The custom domain is pinned via repo Settings → Pages (an artifact `CNAME` is ignored under `actions/deploy-pages` per the GitHub docs); without that Settings value the deploy would follow the maintainer's user-level `mgzwarrior.github.io` → `matt-grant.com` redirect. Separate from the marketing-site Cloudflare Pages deploy (which still hosts the product). A new `tests/test_styleguide_links.py` guard fails CI when any local `href` / `src` in a styleguide card stops resolving so a broken stylesheet or asset surfaces before review. `design/DESIGN_SYSTEM.md`, `design/INTEGRATION.md`, and `README.md` point at the hosted URL.
