...
- Web: **Run-history sidebar** ([#243](https://github.com/mgzwarrior/mgz-pkmn/issues/243)). Collapsible left-rail panel that lists server-side persisted runs from `/api/v1/runs`, surfaces timestamp / row count / total value / tag breakdown per entry, and offers click-to-load (hydrates the editor + results table from `/api/v1/runs/{id}` without re-running the pipeline) and a re-export action that posts to `/api/v1/runs/{id}/export`. Coexists with the existing client-side "Recent searches" panel under the editor — the local panel re-submits the *input*, the sidebar re-loads the *results*. Closes [#58](https://github.com/mgzwarrior/mgz-pkmn/issues/58).
