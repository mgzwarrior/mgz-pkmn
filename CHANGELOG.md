...
- Docs: **Buy Me a Coffee as the preferred funding source** ([#469](https://github.com/mgzwarrior/mgz-pkmn/issues/469)). New "Support the project" section in `README.md` links [Buy Me a Coffee](https://buymeacoffee.com/mgzwarrior) first and [GitHub Sponsors](https://github.com/sponsors/mgzwarrior) second. `.github/FUNDING.yml` gains a `buy_me_a_coffee: mgzwarrior` entry so the repo sidebar's Sponsor button surfaces both options alongside the existing GitHub Sponsors link. The marketing-site footer (`site/src/components/Footer.astro`) adds a Support column with the same two links, and the web SPA footer (`web/src/App.tsx`) gains "Buy me a coffee" and "Sponsor" entries next to the existing GitHub link. GitHub Sponsors stays enabled — this is additive.
