...
- Docs: **Buy Me a Coffee as the preferred funding source** ([#469](https://github.com/mgzwarrior/mgz-pkmn/issues/469)). New "Support the project" section in `README.md` calls out [🍕 Buy me a pizza](https://buymeacoffee.com/mgzwarrior) first and [GitHub Sponsors](https://github.com/sponsors/mgzwarrior) second, and names the three recurring membership tiers (Common / Uncommon / Holo Rare) with a short summary of perks (early-release peeks, monthly roadmap votes, members-only Discussions, supporters-strip placement at Holo Rare). `.github/FUNDING.yml` gains a `buy_me_a_coffee: mgzwarrior` entry so the repo sidebar's Sponsor button surfaces both options alongside the existing GitHub Sponsors link. The marketing-site footer (`site/src/components/Footer.astro`) adds a Support column linking "Buy me a pizza" and "GitHub Sponsors", and the web SPA footer (`web/src/App.tsx`) gains "Buy me a pizza" and "Sponsor" entries next to the existing GitHub link. GitHub Sponsors stays enabled — this is additive.
