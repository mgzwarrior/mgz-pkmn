...
`input/` ships three small, self-contained example lists that together cover
every input syntax and lookup mode the tool supports:
...
The tracked files in `output/` were generated by exactly that command (with
`--no-images` so the PDFs stay small for source control). Drop the flag to
embed card thumbnails — the binder PDF balloons to ~25 MB at full art for
this size of run, but the visual layout is identical.
