...
| `MGZ_PKMN_EBAY_CLIENT_ID` / `MGZ_PKMN_EBAY_CLIENT_SECRET` | API process env | eBay Developer application keys (the OAuth client-credentials pair) for the listing-comps source from [#421](https://github.com/mgzwarrior/mgz-pkmn/issues/421). `EbayAuthClient` ([`src/mgz_pkmn/sources/ebay.py`](../src/mgz_pkmn/sources/ebay.py)) base64-encodes the pair and mints/refreshes an application access token scoped to public data. Both must be set, or use `MGZ_PKMN_EBAY_TOKEN` instead. **When neither is configured the eBay source is disabled with one warning on stderr — the API never crashes and lookups just carry no eBay comps.** See the rotation runbook below. |
| `MGZ_PKMN_EBAY_TOKEN` | API process env | Optional pre-minted application access token. When set it short-circuits the client-credentials mint and is returned verbatim — the simple-deploy escape hatch from [ADR-0020](adr/0020-ebay-pricing-source.md). eBay application tokens are short-lived (~2 h), so this is mostly for local testing; production should set the `CLIENT_ID` / `CLIENT_SECRET` pair and let the client refresh on its own. |
| `MGZ_PKMN_EBAY_ENV` | API process env | `production` (default) or `sandbox`. Selects both the token host and the Browse / Marketplace-Insights API host so sandbox keys never hit production endpoints. |
| `MGZ_PKMN_EBAY_SOLD_ENABLED` | API process env | Truthy (`1`, `true`, `True`) enables the gated **sold**-comps tier (eBay Marketplace Insights API). Off by default because Insights is limited-release — until the application is granted access it returns 403, which the client treats as "no sold comps". Active-listing comps don't need this flag. |
| `MGZ_PKMN_EBAY_VERIFICATION_TOKEN` / `MGZ_PKMN_EBAY_DELETION_ENDPOINT` | API process env | Marketplace account-deletion webhook config from [#609](https://github.com/mgzwarrior/mgz-pkmn/issues/609) ([`api/routes/ebay.py`](../api/routes/ebay.py)). eBay requires a live, verified deletion endpoint before granting production keys. The verification token must be 32–80 chars and match what's entered in the eBay Developer **Alerts & Notifications** portal; the endpoint URL is hashed byte-exact, so `MGZ_PKMN_EBAY_DELETION_ENDPOINT` must equal the registered URL character-for-character. Missing either returns 503 from the challenge handler. |

### Native-app (iOS) session handoff

No new env vars — the native handoff from [#924](https://github.com/mgzwarrior/mgz-pkmn/issues/924) reuses `MGZ_PKMN_SESSION_SECRET`. Any of the provider login routes accept `?next=app` (`/auth/github/login?next=app`, and so on, including `/auth/magic/request`); on success the callback mints a short-lived single-use code and 302s to the fixed `mgzpkmn://auth/callback?code=<code>` scheme instead of setting the browser session cookie — `ASWebAuthenticationSession`'s cookie jar isn't readable by the app, so the cookie-based `302 /` contract the SPA uses doesn't reach a native client. The app then `POST`s the code to `/api/v1/auth/native/exchange`, which burns it and returns `{"session_token": "<cookie value>"}` for the Keychain to store and replay as the `mgz_pkmn_session` cookie on every request. Codes expire in 60 seconds and are single-use; the exchange endpoint is unauthenticated by design (it's how the session is acquired). Rate-limiting it is tracked alongside the broader abuse-hardening work in [#710](https://github.com/mgzwarrior/mgz-pkmn/issues/710).

### eBay token rotation runbook

The eBay source authenticates with an **application** OAuth client-credentials pair (there's no per-user eBay login — see [ADR-0020](adr/0020-ebay-pricing-source.md)). Rotation means replacing the `MGZ_PKMN_EBAY_CLIENT_SECRET` (and, if regenerated, `MGZ_PKMN_EBAY_CLIENT_ID`) in the Render dashboard. The runtime access token mints and refreshes itself, so there's nothing to rotate on the token side — only the developer keys.

Rotate when a key is suspected leaked, when an eBay keyset is regenerated in the portal, or on a routine cadence per your security policy.

1. **Mint a new keyset** in the eBay Developer portal under *Application Keysets* (<https://developer.ebay.com/my/keys>). Generating a new client secret in the portal **invalidates the old secret immediately**, so do this right before step 2 to minimise the window where the deployed key is stale.
2. **Update the Render secret** — in the `mgz-pkmn` service's *Environment* tab, set `MGZ_PKMN_EBAY_CLIENT_SECRET` (and `MGZ_PKMN_EBAY_CLIENT_ID` if it changed) to the new value. Both are declared `sync: false` in [`render.yaml`](../render.yaml), so their values live only in the dashboard.
3. **Redeploy** so the new value is picked up. An env-var change doesn't always trigger an automatic redeploy; use *Manual Deploy → Deploy latest commit*, or the *Deploy* GitHub Action (see [Forced rebuilds](#forced-rebuilds-when-needed)).
4. **Verify** the cutover. The cleanest signal is the absence of the `! eBay comps disabled` warning in the service logs plus eBay comps reappearing in a lookup; if the new keyset is wrong, lookups simply carry no eBay comps (the source fails soft) rather than erroring.

If you ever need to **disable** the eBay source entirely (e.g. an extended portal outage), clear `MGZ_PKMN_EBAY_CLIENT_ID` / `MGZ_PKMN_EBAY_CLIENT_SECRET` / `MGZ_PKMN_EBAY_TOKEN` and redeploy. The API logs a single `! eBay comps disabled` line on the first lookup and serves every lookup without eBay comps — it will not crash or 500. Re-supply the keys and redeploy to turn it back on.

The marketplace account-deletion webhook (`MGZ_PKMN_EBAY_VERIFICATION_TOKEN` / `MGZ_PKMN_EBAY_DELETION_ENDPOINT`) is **independent** of the pricing keyset — rotating the developer keys above does not touch it, and vice versa.
