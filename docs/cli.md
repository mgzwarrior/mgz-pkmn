...
|---|---|
| Hit rate limit on pokemontcg.io | Set `POKEMONTCG_IO_API_KEY` so the upstream limit jumps from the anonymous tier to roughly 20k requests/day, then rerun the lookup. |
| Card not found / unmatched | Check spelling, add a set hint if the name is ambiguous, and inspect the `highlights.missing` section of the JSON report to see which lines failed. |
| PriceCharting scrape error | Verify the product URL still resolves, then add or update a sticky override in `url_overrides.json` as described in [Cache](cache.md). |
| Disk cache is huge or obviously wrong | Temporarily bypass caching with `MGZ_PKMN_NO_CACHE=1`, or delete `~/.cache/mgz-pkmn/` and rerun so the CLI repopulates it from scratch. |
| Stale prices or set metadata | Cache TTLs are documented in [Cache](cache.md); delete the relevant cache file if you need to force an immediate refresh. |
| Network or API failures abort the run | Retry with `-v` to see the failing request, then confirm your network connection and that the upstream service is reachable before opening an issue. |
