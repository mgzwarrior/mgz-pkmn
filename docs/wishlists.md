...
| Table | Columns |
|-------|---------|
| `wishlists` | `id`, `user_id`, `name`, `description`, `created_at` |
| `wishlist_items` | `id`, `wishlist_id`, `card_json`, `notes`, `max_price`, `added_at` |

`max_price` uses the same `Numeric(12, 2)` precision as `run_rows.market_price` so future alerting can join the two without type coercion. `card_json` carries the matched payload verbatim — same shape the lookup pipeline produces and the same shape collections use.
