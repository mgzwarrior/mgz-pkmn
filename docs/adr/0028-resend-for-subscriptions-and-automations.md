...
- Resend only accepts a contact `properties` key that has been **pre-defined on the audience** — setting an unknown key fails the whole create. So `reason` must exist as a **string** Contact Property before the first signup, or every subscribe 502s. This is operator setup (a one-time dashboard/API step in the runbook below), the same shape as the audience, the API key, and the automation — not something the route provisions per request.
...
5. **Build the Automation** under *Automations*: trigger = *contact added to* the audience; add a branch/condition on the `reason` property; create three branches (`collector`, `show`, `builder`), each with three email steps + sensible delays. Paste the copy from [docs/marketing/emails/](../../docs/marketing/emails/) (subjects + preheaders are in each file's front-matter).
6. **Smoke test**: submit the live form once per reason; confirm a contact lands in the audience with the right `reason` property and the correct track begins.
