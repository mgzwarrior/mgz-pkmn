...
- A new server route, **`POST /api/v1/subscribe`** ([api/routes/subscribe.py](../../api/routes/subscribe.py)), accepts `{ email, reason }` where `reason ∈ {collector, show, builder}` and does **two writes** to Resend. (1) It creates a contact in a single Resend **audience** (`RESEND_AUDIENCE_ID`), stamping the reason into Resend's custom `properties` map (`properties.reason`) — the durable record that keeps segmentation, exports, and unsubscribe unified. (2) It fires a **`New Signup` custom event** (`POST /events/send`) carrying the reason in the event `payload`, which is what actually triggers the welcome Automation. The Resend API key (`RESEND_API_KEY`) lives only on the backend. Both env vars are `sync: false` secrets in [render.yaml](../../render.yaml); the route returns 503 when either is unset (a deploy-time signal, never a runtime leak) and 502 when either Resend call rejects or is unreachable.
- Resend only accepts a contact `properties` key that has been **pre-defined on the audience** — setting an unknown key fails the whole create. So `reason` must exist as a **string** Contact Property before the first signup, or the contact-create 502s. This is operator setup (a one-time dashboard/API step in the runbook below), the same shape as the audience, the API key, and the automation — not something the route provisions per request.
...
- A **single Resend Automation** (built once in the dashboard — see runbook) triggers on the **`New Signup` custom event** and **branches on `event.reason`** into three paths, each sending that track's three emails. The email copy is authored in [docs/marketing/emails/](../../docs/marketing/emails/) (`collector/`, `show-prep/`, `builder/`) and pasted into the Automation steps. We use a custom-event trigger (not "contact added to the audience") so the drip is decoupled from audience membership — adding a contact for segmentation doesn't double as a send signal, and the event re-fires cleanly when re-testing. Event-triggered Automations surface payload data under the `event.*` namespace, which is why the branch reads `event.reason`, not the contact's `properties.reason` (which an event trigger doesn't look at).

One audience, three branches: the reason is both data on the contact (for segmentation, exports, and a unified unsubscribe surface) and the event payload that drives the branch.
...
4. **Create the `reason` Contact Property** on that audience (*Audiences → Properties*, or via the API): key `reason`, type **string**. This is required *before* the first signup — Resend rejects a contact create that sets an undefined property key, so without it every `POST /subscribe` returns 502 and no one enters the drip. (A fallback value like `collector` is optional.)
5. **Build the Automation** under *Automations*: trigger = a **custom event** with `event_name` `New Signup` (this must match `RESEND_SIGNUP_EVENT` in [api/routes/subscribe.py](../../api/routes/subscribe.py) verbatim — if they drift, signups land as contacts but the drip never fires). Add a branch/condition on **`event.reason`** (the payload field, exposed under the `event.*` namespace — *not* the contact's `properties.reason`); create three branches (`collector`, `show`, `builder`), each with three email steps + sensible delays. Paste the copy from [docs/marketing/emails/](../../docs/marketing/emails/) (subjects + preheaders are in each file's front-matter).
6. **Smoke test**: submit the live form once per reason with a fresh email each time; confirm a contact lands in the audience with the right `reason` property, the Automation logs a run, and the correct track begins.
