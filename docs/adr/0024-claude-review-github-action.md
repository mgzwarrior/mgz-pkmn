...
**Permissions.** The workflow grants only `contents: read`, `pull-requests: write`, and `issues: write` — enough for the action to read the diff and post the review, nothing else. The action step also passes `github_token: ${{ secrets.GITHUB_TOKEN }}` explicitly: without that input, `anthropics/claude-code-action`'s `setupGitHubToken` falls back to an OIDC-exchange path that mints a Claude GitHub App installation token with Contents Read & Write, which would silently widen the runner past this job's `contents: read` (#534). Passing the workflow's own `GITHUB_TOKEN` keeps the runner constrained to exactly the declared permissions, and also lets us avoid granting `id-token: write` since the OIDC path is no longer taken.
