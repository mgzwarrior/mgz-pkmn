...
- **Revisit if the `mgz-pkmn-vendor` plugin surface (a separate, not-yet-published package) reaches PyPI** — `designing-python-apis` (semver-breaking-change discipline, deprecation strategy — `mgz-pkmn` itself doesn't expose a documented public API surface beyond its CLI/dataclasses) and `documenting-python-libraries` (Sphinx/ReadTheDocs; this repo's docs are hand-written Markdown, not a generated API reference).
- **Skip as duplicative** — `setting-up-python-libraries` (project is already scaffolded), `improving-python-code-quality` (ruff and the radon MI gate already enforce this in CI), `packaging-python-libraries` (`mgz-pkmn` is already published to [PyPI](https://pypi.org/project/mgz-pkmn/) via `release.yml`'s trusted-publishing pipeline — see [docs/contributing.md](../contributing.md#pypi-trusted-publisher-wiring) — so this would just re-describe an already-automated flow), `managing-python-releases` (release-please plus the `cut-release` skill already own this), `optimizing-python-performance` (redundant with the installed `performance-profiler` skill), `building-python-communities` (redundant with `docs/contributing.md`, `AGENTS.md`, and the `groom-issues` skill).
