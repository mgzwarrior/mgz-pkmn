...
- **Cache-only.** `MGZ_PKMN_CACHE_ONLY=1` pins the API offline: a disk-cache miss degrades to an empty `MISS-CACHE-ONLY` result instead of fetching pokemontcg.io. This is the belt to the cassette's braces. The cassette covers the cards the specs touch, but the Swipe deck (the default surface) samples a *random* set from the bundled catalog on load, and one outside the cassette would otherwise fetch `/api/v1/sets/{id}/cards` live in the background. The flag guarantees zero outbound calls regardless of which set the deck picks. It's off-default, so it changes nothing for production or self-host — see [`api/cache_mode.py`](../api/cache_mode.py).
