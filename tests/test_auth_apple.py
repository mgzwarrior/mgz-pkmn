...
- ``/auth/apple/callback`` is a **POST** (Apple uses
  ``response_mode=form_post``) rather than a GET.
...
- ``/auth/apple/login`` 404s when the auth scaffold is off.
- ``/auth/apple/login`` returns 503 when any Apple env var is missing.
- Callback: state mismatch (``OAuthError``) → 400.
- Callback: ``id_token`` signature mismatch → 400.
- Callback: ``id_token`` missing the ``email`` claim → 400.
- Callback: ``id_token`` missing ``sub`` → 400.
...
class CallbackErrorTests(_IsolatedDbMixin):
    """Callback handler error branches. Patches Authlib /
    ``fetch_apple_jwks`` to drive each failure mode without standing
    up a real OAuth server."""
    ...
    def test_callback_apple_returned_error_field(self) -> None:
        ...
        # request is malformed; we short-circuit to 400 without hitting
        # the token endpoint.
        from api.main import app
        ...
        with TestClient(app) as client:
            r = client.post(
                "/api/v1/auth/apple/callback",
                data={"error": "user_cancelled_authorize", "state": "anything"},
                follow_redirects=False,
            )
            self.assertEqual(r.status_code, 400)
            self.assertEqual(r.json()["detail"], "oauth_failed")
    ...
    def test_callback_missing_sub_returns_400(self) -> None:
        ...
        with (
            ...
            self.assertEqual(r.status_code, 400)
            self.assertEqual(r.json()["detail"], "no_apple_sub")
...
class CallbackHappyPathTests(_IsolatedDbMixin):
    ...
    def _drive_callback(
        ...
        *,
        ...
        token = {"access_token": "x", "id_token": token_str, "token_type": "bearer"}
        data: dict[str, str] = {"code": "abc", "state": "anything"}
