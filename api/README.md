...
endpoint every 30 seconds (with a 10-second startup grace period), so
Render and other runtimes that read the OCI health field surface the
container as `(healthy)` once the app is ready.
