...
- `session.engine` / `session.SessionLocal` are the long-lived engine and
  session factory; `session.get_db` is the FastAPI request-scoped dep.
...
- `serialize.row_to_columns` / `row_to_summary` convert in-memory `Row`s to
  the persisted form (promoted columns + opaque JSON sub-payloads).
"""
